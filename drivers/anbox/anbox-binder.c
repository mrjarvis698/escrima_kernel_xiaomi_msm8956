...
#include "anbox-binder.h"
//#include "anbox-binder_trace.h"
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_ACQUIRE_DONE: {
			...
			if (get_user(node_ptr, (uint32_t __user *)ptr))
				return -EFAULT;
		...
		case BC_FREE_BUFFER: {
			...
			if (get_user(data_ptr, (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(binder_uintptr_t);
...
static int __init init_binder_device(int idx)
{
	 ...
	 */
	char numstr[12 + 15] = "anbox-binder";
	...
	if (idx != 0)
		ret = snprintf(numstr, sizeof(numstr), "anbox-binder%d", idx);
...
static int __init binder_init(void)
{
	...
	binder_debugfs_dir_entry_root = debugfs_create_dir("anbox-binder", NULL);
	...
	ret = register_chrdev_region(MKDEV(BINDER_DKMS_MAJOR, 0),
				     BINDER_DKMS_MAX_MINOR, "anbox-binder");
	if (ret)
		goto on_error_remove_debugfs;

	binder_class = class_create(THIS_MODULE, "anbox-binder");
